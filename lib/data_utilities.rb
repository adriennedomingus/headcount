class DataUtilities
  ...
  def self.load_children_in_poverty_data(economic_profile_objects)
    @cip_contents.each do |row|
      ...
      matching_epo.data[:children_in_poverty][row[:timeframe].to_i] = truncate_value(row[:data].to_f)
    end
  end
  ...
  def self.load_frl_data(economic_profile_objects)
    @frl_contents.each do |row|
      ...
      if row[:dataformat] == "Percent"
        matching_epo.set_free_or_reduced_price_lunch_percentage(row[:timeframe].to_i, truncate_value(row[:data].to_f))
  ...
  def self.load_title_i_data(economic_profile_objects)
    @ti_contents.each do |row|
      ...
      match.data[:title_i][row[:timeframe].to_i] = truncate_value(row[:data].to_f)
    end
  end
  ...
  def self.match_enrollment_data_to_district_data(enrollment_objects, success, row)
    enrollment_objects.each do |enrollment|
      if row[:location].upcase == enrollment.name.upcase
        enrollment.data[success][row[:timeframe].to_i] = truncate_value(row[:data].to_f)
      end
    end
  end
    ...
    statewide_objects.each do |statewide|
      if row[:location] == statewide.name
        ...
        if UNDEFINED_DATA.include?(row[:data])
          ...
          statewide.data[data_type][row[:timeframe].to_i][subject] = truncate_value(row[:data].to_f)
        end
      end
    end
  end
